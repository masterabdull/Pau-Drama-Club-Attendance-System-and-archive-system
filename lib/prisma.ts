...
function matches(row: Row, where?: Row): boolean { if (!where) return true; if (where.OR) return where.OR.some((item: Row) => matches(row, item)); return Object.entries(where).every(([key, expected]) => { if (expected && typeof expected === "object" && "contains" in expected) return String(row[key] || "").toLowerCase().includes(String(expected.contains).toLowerCase()); if (expected && typeof expected === "object" && "not" in expected) return row[key] !== expected.not; if (expected && typeof expected === "object" && ("gte" in expected || "gt" in expected || "lte" in expected || "lt" in expected)) { const actual = new Date(row[key]).getTime(); return (!expected.gte || actual >= new Date(expected.gte).getTime()) && (!expected.gt || actual > new Date(expected.gt).getTime()) && (!expected.lte || actual <= new Date(expected.lte).getTime()) && (!expected.lt || actual < new Date(expected.lt).getTime()); } return row[key] === expected; }); }
...
class LocalRepository {
  ...
  async member_findMany(args: Row = {}) { const store = await load(); let rows = ordered(store.members.filter((row) => matches(row, args.where)), args.orderBy, args.take); if (args.include?.attendance) rows = rows.map((row) => ({ ...row, attendance: store.attendance.filter((item) => item.memberId === row.id).map((item) => args.include.attendance.include?.session ? { ...item, session: store.sessions.find((session) => session.id === item.sessionId) } : item) })); return rows; }
  ...
  async member_update(args: Row) { const store = await load(); const index = store.members.findIndex((row) => row.id === args.where.id); if (index < 0) throw new Error("Member not found"); store.members[index] = { ...store.members[index], ...args.data, updatedAt: new Date().toISOString() }; await save(store); return store.members[index]; }
  ...
  async session_delete(args: Row) { const store = await load(); const index = store.sessions.findIndex((row) => row.id === args.where.id); if (index < 0) throw new Error("Session not found"); store.sessions.splice(index, 1); store.attendance = store.attendance.filter((item) => item.sessionId !== args.where.id); await save(store); return { id: args.where.id }; }
